...
import { Prompt } from '../storage/promptStorage';
...
export async function processPromptsOnServer(prompts: Prompt[]): Promise<ServerProcessingResult> {
  ...
  for (const prompt of prompts) {
    try {
      ...
      const promptFull = constructPropmt(prompt.prompt);
      const chatGptResponse = await sendToGeminiModel(promptFull);
      ...
      const emailResult = await sendEmail({
        ...
        html: `
          ...
          <hr />
          <div>${chatGptResponse.content}</div>
          <hr />
